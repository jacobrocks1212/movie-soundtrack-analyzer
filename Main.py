...
with open('movie_data.csv', mode='w') as movie_file:
    ...
    while(num_data_points < 3000):
        ...
        while(not valid_movie):
            # ranInt = random.randint(0, 10000), 229500
            try:
                movie = ia.get_movie(329100 + counter)
                valid_movie = True
                counter += 1
            except:
                counter += 1
        ...
        if(has_genre_info and has_soundtrack_info):
            ...
            for k in range(len(soundtracks)):
                ...
                # handling 401 error (need to refresh access token)
            ...
            if(num_audio_features > 0):
                ...
                # handling comma in movie title writing issue
                ...
                # writing songs and artists
                for k in range(len(soundtracks)):
                    movie_file.write(
                        "," + soundtracks[k][0].replace(",", ".").replace("\"", "") + ',' + soundtracks[k][1].replace(",", ".").replace("\"", ""))
